...
def get_gan_info(model, img_path):
    result = dict()
    ...
    try:
        if gan:
            ...
            dis = (3.1 * f) / (gan['x2y2'][0] - gan['x1y1'][0])
            result['dis_forward'] = dis  # 距离杆的距离
            dis_cm = 3.1 / (gan['x2y2'][0] - gan['x1y1'][0])  # 每像素多少厘米，需要用相似三角形计算
            if int(gan['x1y1'][0]) < 10:
                result['code'] = 'action'
                result['direction'] = 'left'
                result['distance'] = 30
                result['finish'] = False
                result['dis_forward'] = 0
                result['msg'] = '杆在左边界，测量可能不准确，需向 左  飞行后重新拍照计算'
                return result
            elif int(gan['src_img_size'][1]) == (gan['x2y2'][0]):
                result['code'] = 'action'
                result['direction'] = 'right'
                result['distance'] = 30
                result['finish'] = False
                result['dis_forward'] = 0
                result['msg'] = '杆在右边界，测量可能不准确，需向  右  飞行重新拍照计算'
                return result
            # 计算距离杆中心的距离
            diff = (gan['src_img_size'][1] - gan['x2y2'][0] - gan['x1y1'][0]) / 2
